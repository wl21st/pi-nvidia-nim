...
const NVIDIA_API_KEY_ENV_NAMES = [
...
const CONTEXT_WINDOWS: Record<string, number> = {
  ...
  "nvidia/nemotron-4-340b-instruct": 262144,
...
function isNimApiKeyEnvValue(value: string): boolean {
  return NVIDIA_API_KEY_ENV_NAMES.some(
    (envName) => process.env[envName]?.trim() === value,
  );
}

function isNimApiKeyCredential(
  credential: unknown,
): credential is NimApiKeyCredential {
  return (
...
function getStoredNimApiKeyConfig(
...
function getStoredResolvedNimApiKey(
  authStorage?: AuthStorageLike,
): string | undefined {
...
function normalizeResolvedNimApiKey(
...
function resolveNimApiKey(
  ...
  if (
    ...
    (!resolvedApiKey ||
      isNimApiKeyEnvPlaceholder(resolvedApiKey) ||
      isNimApiKeyEnvValue(resolvedApiKey))
  ) {
  ...
  if (
    storedApiKey &&
    (!resolvedApiKey || isNimApiKeyEnvPlaceholder(resolvedApiKey))
  )
    return storedApiKey;
...
function mapNimTopLevelReasoning(
...
function mapDeepSeekV4Reasoning(
  reasoning: SimpleStreamOptions["reasoning"],
): "high" | "max" {
...
function buildNimRequestHeaders(
...
function nimStreamSimple(
  ...
  if (
    thinkingConfig &&
    isThinkingEnabled &&
    !thinkingConfig.sendReasoningEffort
  ) {
    ...
    onPayload: (params: unknown) => {
      ...
      if (thinkingConfig) {
        if (isThinkingEnabled) {
          ...
          p.chat_template_kwargs = buildThinkingKwargs(
            thinkingConfig,
            reasoning,
          );
...
type NimModelFetchResult =
  ...
  | {
      ok: false;
      reason: "auth" | "transient" | "invalid" | "network" | "other";
    };
